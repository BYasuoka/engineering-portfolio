...
def build_scenario_data(gross_income, federal_brackets, state_brackets, roth_annual_limit):
    biweekly_roth = roth_annual_limit / PAY_PERIODS_PER_YEAR
    ...
    return [
        {
            ...
            "biweekly_401k": 0.0,
            "biweekly_roth": 0.0,
            "biweekly_net": net_no_retirement / PAY_PERIODS_PER_YEAR,
        },
        {
            "name": "8% 401(k) only",
            "biweekly_401k": annual_401k / PAY_PERIODS_PER_YEAR,
            "biweekly_roth": 0.0,
            "biweekly_net": net_401k_only / PAY_PERIODS_PER_YEAR,
        },
        {
            "name": "Roth IRA only",
            "biweekly_401k": 0.0,
            "biweekly_roth": biweekly_roth,
            "biweekly_net": net_roth_only / PAY_PERIODS_PER_YEAR,
        },
        {
            "name": "8% 401(k) + Roth IRA",
            "biweekly_401k": annual_401k / PAY_PERIODS_PER_YEAR,
            "biweekly_roth": biweekly_roth,
            "biweekly_net": net_both / PAY_PERIODS_PER_YEAR,
        },
    ]
...
def build_scenario_rows(scenarios):
    ...
    for scenario in scenarios:
        rows.append(
            [
                scenario["name"],
                format_currency(scenario["biweekly_401k"]),
                format_currency(scenario["biweekly_roth"]),
                format_currency(scenario["biweekly_net"]),
            ]
        )
    return rows
...
def build_budget_rows(scenarios):
    ...
    rows = []
    for scenario in scenarios:
        biweekly_net = scenario["biweekly_net"]
        for label, needs_pct, wants_pct, savings_pct in budget_splits:
            rows.append(
                [
                    scenario["name"],
                    label,
                    format_currency(biweekly_net * needs_pct),
                    format_currency(biweekly_net * wants_pct),
                    format_currency(biweekly_net * savings_pct),
                ]
            )
    return rows
